...
namespace advent_of_code_2020.Days {
    public class Day22 : Day<long> {
        ...
        private Player PlayGame(Player[] players, bool withSubGames) {
            ...
            while (players.All(p => p.Cards.Count > 0)) {
                ...
                if (withSubGames && players.All(p => p.Cards.Count > p.Cards.First())) {
                    var subGameWinner = PlayGame(players.Select(p => new Player { Id = p.Id, Cards = p.Cards.Skip(1).Take(p.Cards.First()).ToList() }).ToArray(), withSubGames);
